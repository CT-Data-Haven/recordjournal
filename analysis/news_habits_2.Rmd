...
geos <- bind_rows(geo_age, geo_latino, geo_language, geo_total) %>% 
  ...
  mutate(category = as.factor(category) %>%
           fct_relevel(., "total", "age", "language", "latino"),
...
geos %>% 
  ...
  labs(title = "Regions of interest by category and group",
       subtitle = "Regional = New England/NYC Metro", x = NULL, y = NULL)
```
...
top_latino <- read %>%
  filter(!is.na(latino), !is.na(topics_of_interest)) %>%
  select(group = latino, topic = topics_of_interest) %>% 
  mutate(category = "latino") %>% 
  separate_rows(topic, sep = ";")
...
topics <- bind_rows(top_age, top_latino, top_language, top_total) %>% 
  select(category, group, everything()) %>%  
...
I'm really surprised no young people selected environment/climate change. Music/arts/culture, sports, and technology top the list for young people. Health, education, and music/arts/culture for the other two groups.
...
Local events standing out among people who don't identify as Latino is interesting.

```{r top_latino}
topics %>% 
  filter(!is.na(share), category == "latino") %>% 
  ggplot(aes(x = share, y = reorder(topic, share))) +
  geom_col(width = .8) +
  geom_text(aes(label = scales::percent(share, accuracy = 1)), hjust = 1.2) +
  facet_grid(cols = vars(group)) +
  theme(axis.text.x = element_blank()) +
  labs(title = "Topics of interest (by ethnicity)", subtitle = "Yes/No refers to whether respondent identifies as Latino/Latina/Latinx", x = NULL, y = NULL)
```

Looks like people who speak English at home also have an interest in local events, otherwise health, education, and music top the lists still.
...
spec_latino <- read %>%
  filter(!is.na(latino), !is.na(special_stories)) %>%
  select(group = latino, story = special_stories) %>% 
  mutate(category = "latino") %>% 
  separate_rows(story, sep = ";")
...
spec <- bind_rows(spec_age, spec_latino, spec_language, spec_total) %>% 
  select(category, group, everything()) %>% 
...
spec %>% 
  ...
  ggplot(aes(x = share, y = fct_rev(group))) +
  geom_col(width = .8) +
  geom_text(data = . %>% filter(share >= .3), aes(label = scales::percent(share, accuracy = 1)), hjust = 1.2) +
    geom_text(data = . %>% filter(share < .3), aes(label = scales::percent(share, accuracy = 1)), hjust = -.2, color = "black") +
