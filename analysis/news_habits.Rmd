...
```{r}
eng_under40 <- read_english %>% left_join(age, by = "id") %>% filter(age == "under_40")
spn_under40 <- read_spanish %>% left_join(age, by = "id") %>% filter(age == "under_40")
eng_over40 <- read_english %>% left_join(age, by = "id") %>% filter(age == "over_40")
spn_over40 <- read_spanish %>% left_join(age, by = "id") %>% filter(age == "over_40")
```

```{r}
eng_list_under40 <- str_split(eng_under40$Q10, ",| y | and | or|\\/|  ", simplify = T)
...
cloud_under40 <- x %>% group_by(source) %>% count()
...
ggplot(cloud_under40, aes(label = source, size = n+3, color = n)) +
  ...
  labs(title = "Sources preferred by people under 40")

ggplot(cloud_over40, aes(label = source, size = n+3, color = n)) +
  geom_text_wordcloud_area(family = "Barlow Semi Condensed", eccentricity = .6) +
  scale_size_area(max_size = 10) +
  theme_minimal() +
  scale_color_gradient(high = qual_pal[1], low = qual_pal[7]) +
  labs(title = "Sources preferred by people over 40")
```
