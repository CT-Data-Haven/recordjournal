...
read_english <- read_english %>% select(-Q1, -Q29, -Q30, -Q31) %>% remove_empty("rows")
read_spanish <- read_spanish %>% select(-Q1, -Q29, -Q30, -Q31) %>% remove_empty("rows")
```
...
read_english_pr <- read_english_pr %>% select(-Q1) %>% remove_empty("rows")
read_spanish_pr <- read_spanish_pr %>% select(-Q1) %>% remove_empty("rows")

read_english <- read_english %>% bind_rows(read_english_pr)
read_spanish <- read_spanish %>% bind_rows(read_spanish_pr)
```
...
response_gender <- tibble(gender = read_english$Q23,
  ...
  bind_rows(tibble(gender = read_spanish$Q23,
   ...
   mutate(gender = if_else(gender == "Hombre", "Man", gender),
          ...
          gender = if_else(gender == "Transgénero", "Transgender", gender)) %>% 
...
The vast majority are still in Meriden and Wallingford but now we also have a couple from Mass and several other towns around CT.
